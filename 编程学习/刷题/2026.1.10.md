...
```
### 队列也得是数组或者链表的结构。`pair<int, int>& lhs` 这是一对数，`lhs.first`
`lhs.second`分别代表第一个数与第二个数。
...
```
因为他们不是连续的，所以必须用迭代器的方法。
- `it` 是一个指针（迭代器）。
    
- `*it` 是**解引用**操作，它取出了指针指向的**内容**。 map(数字， 频率).
### priority_queue为top();普通是front();
...
### 错误点
1. 最后应该是`i--`；
2. `result`数组要么事先`k`；要么就使用`push_back()`;而不是使用`result[i] `进行赋值。