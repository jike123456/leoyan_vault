...
{	
	...
	My_SI2C_SendBytes(&si2c, 0x78, commands, 5);//给屏幕78发数据、send开亮命令
	
	uint8_t rcvd;//定义一个接受区
	
	My_SI2C_ReceiveBytes(&si2c, 0x78, &rcvd, 1);//接收78的传来的rcvd
	//判断位运算，看屏幕是亮是灭
	if((rcvd & (0x01 << 6)) == 0)//只有第六位是起作用的，&进行第六位判断
	{
		GPIO_WriteBit(GPIOC, GPIO_Pin_13, Bit_RESET);//低电平亮 0 & 1
	}
	else
	{
		GPIO_WriteBit(GPIOC, GPIO_Pin_13, Bit_SET);//高电平灭 1 & 1
	} 
	while(1)
	{
...
void My_OnBoardLED_Init(void)
{
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);//使能时钟，打开电源
	
	GPIO_InitTypeDef GPIO_InitStruct;//定义结构体
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_13;//填表，引脚
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_Out_OD;//填表，模式
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_2MHz;//填表，速度
	GPIO_Init(GPIOC, &GPIO_InitStruct);//进行初始化
	
	GPIO_WriteBit(GPIOC, GPIO_Pin_13, Bit_SET);//初始让他是熄灭状态。
}

```
这段代码是一个非常典型的 **“配置 -> 执行 -> 反馈”** 的嵌入式程序结构。它展示了如何用 C 语言操作 STM32 的硬件资源（GPIO、I2C协议）来控制外部设备（OLED）和板载资源（LED）。给Oled发送命令数组， 判断直接用GPIO进行控制Led的亮灭

**配置好 I2C 接口，命令 OLED 屏幕全亮，然后检查屏幕的一个状态位，如果该位为 0，就点亮板子上的 LED 灯。**
