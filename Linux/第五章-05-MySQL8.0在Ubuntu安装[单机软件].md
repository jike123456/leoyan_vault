# 第五章-05-MySQL8.0在Ubuntu安装[单机软件]

MySQL 8.0是MySQL数据库的最新主要版本，带来了许多性能优化、安全增强和新特性。本节将详细介绍在Ubuntu系统上安装MySQL 8.0社区版的步骤。

## 1. 环境准备

*   **操作系统：** Ubuntu Server 18.04/20.04 (或更高版本)
...
访问MySQL官网（dev.mysql.com/downloads/repo/apt/）获取最新版本的`.deb`包下载链接。这里以MySQL 8.0为例。
```bash
...
**重要：选择认证方法**
在安装过程中，你可能会遇到一个关于认证方法的选择。
*   **`Use Strong Password Encryption for Authentication (RECOMMENDED)` (推荐)**：使用`caching_sha2_password`插件，这是MySQL 8.0的默认认证方式，安全性更高。但旧的客户端可能不支持。
*   **`Use Legacy Authentication Method (Retain MySQL 5.x Compatibility)`：** 使用`mysql_native_password`插件，兼容MySQL 5.x及更早版本的客户端。

建议选择第一项（推荐的强密码加密）。如果你的应用或客户端不支持`caching_sha2_password`，则后续可能需要手动更改用户认证方式。
...
CREATE USER 'remote_user'@'%' IDENTIFIED BY 'YourRemotePassword!' WITH authentication_plugin='caching_sha2_password';
...
```
**注意：**
*   `'%'` 表示允许从任何主机连接，你也可以指定特定的IP地址，例如 `'192.168.1.100'`。
*   `WITH authentication_plugin='caching_sha2_password'` 指定了认证插件。如果你之前在安装时选择了传统认证方法，这里也应保持一致。
...
至此，你已成功在Ubuntu上安装并配置了MySQL 8.0数据库。你可以通过命令行客户端或图形化工具连接到数据库，并开始你的数据库操作。
