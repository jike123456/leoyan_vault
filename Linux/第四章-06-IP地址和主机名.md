...
```
- `eth0`: 网络接口的名称。
- `inet 192.168.1.10/24`: 这是最重要的信息，`192.168.1.10` 是该接口的IPv4地址。`/24` 是CIDR表示法，代表子网掩码为 `255.255.255.0`。
- `inet6 ...`: IPv6地址。

#### (2) `ifconfig` 命令 (传统方法)
`ifconfig` 是一个较旧的工具，在许多新的Linux发行版中可能需要手动安装 (`net-tools` 包)。
