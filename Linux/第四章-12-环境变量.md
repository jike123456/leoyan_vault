...
*   **`/etc/profile`：**
    ==这个文件在==所有用户登录时执行，用于设置==全局环境变量和执行全局登录脚本==。
    同样vim进入文件==/etc/profile==；必须==以sudo权限==，否则==只读，：q！退出==
    ```bash
    # 在文件末尾添加
    export SYSTEM_WIDE_VAR="Global Value"
    ```
    修改后需要==重启系统或手动== `source /etc/profile`。
...
- 示例：创立一个随处==可以执行==的脚本：自定义环境变量PATH
![500](assets/第四章-12-环境变量/file-20251216153053635.png)
[chmod数字](../Linus常见命令.md#^89f4a4)
[.././~](../Linus常见命令.md#^0223b4)
## 4. 删除环境变量
