...
*   **解压 `.tar.gz` ==文件到指定目录==：**
    ```bash
    tar -zxvf archive.tar.gz -C /tmp/my_extracted_files/
    ```
...
`zip` 和 `unzip` 命令用于创建和提取 `.zip` 格式的归档文件。这种格式在Windows系统中非常常见，因此在==Linux和Windows之间交换文件时特别有用。==
**基本语法：**
```bash
