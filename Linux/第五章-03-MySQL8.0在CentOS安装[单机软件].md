# 第五章-03-MySQL8.0在CentOS安装[单机软件]

MySQL 8.0是MySQL数据库的最新主要版本，带来了许多性能优化、安全增强和新特性。本节将详细介绍在CentOS系统上安装MySQL 8.0社区版的步骤。
...
访问MySQL官网（dev.mysql.com/downloads/repo/yum/）获取最新版本的RPM包下载链接。这里以MySQL 8.0为例。
```bash
...
默认情况下，MySQL Yum存储库会启用最新稳定版。如果你之前安装过其他版本，需要确保MySQL 8.0被启用。

编辑 `/etc/yum.repos.d/mysql-community.repo` 文件：
```bash
sudo vi /etc/yum.repos.d/mysql-community.repo
```
找到 `[mysql80-community]` 部分，确保 `enabled=1`。
找到 `[mysql57-community]` 或其他版本部分，确保 `enabled=0`。
...
## 5. 初始化MySQL并获取临时密码（重要步骤！）

MySQL 8.0在首次启动时会自动进行初始化，并为`root`用户生成一个临时密码。你需要找到这个密码来登录MySQL并设置新密码。
...
登录成功后，MySQL会强制你修改`root`密码。MySQL 8.0默认启用了密码策略验证插件（`validate_password`），要求密码必须包含大小写字母、数字、特殊字符，并且长度至少为8位。
...
set global validate_password.policy=LOW;   # 设为LOW，只检查长度
set global validate_password.length=6;  # 设为最小长度，例如6

# 再次修改密码（使用简单密码）
ALTER USER 'root'@'localhost' IDENTIFIED BY 'newpassword';

# 恢复密码策略 (强烈建议在生产环境恢复)
set global validate_password.policy=MEDIUM; # 设为MEDIUM
set global validate_password.length=8; # 设为8
```
**注意：** MySQL 8.0中 `validate_password` 插件的变量名有所变化，例如 `validate_password_policy` 变为 `validate_password.policy`。
...
至此，你已成功在CentOS上安装并配置了MySQL 8.0数据库。你可以通过命令行客户端或图形化工具连接到数据库，并开始你的数据库操作。
